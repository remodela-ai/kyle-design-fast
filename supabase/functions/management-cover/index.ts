...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Project cover generated, persisting to storage...");
    
    // Persist the image to Supabase Storage
    const imageUrl = await persistImageToStorage(tempImageUrl, `cover-${sessionId}`);
    
    console.log("✅ Project cover persisted successfully");
