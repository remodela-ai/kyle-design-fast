...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Elevation view generated, persisting to storage...");
     
    // Persist the image to Supabase Storage
    const imageUrl = await persistImageToStorage(tempImageUrl, `elevation-${sessionId}`);
     
    console.log("✅ Elevation view persisted successfully");
