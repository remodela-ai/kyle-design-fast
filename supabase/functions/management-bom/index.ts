...
serve(async (req) => {
  ...
  try {
    ...
    console.log("BOM generated, persisting to storage...");
    
    // Persist the image to Supabase Storage
    const imageUrl = await persistImageToStorage(tempImageUrl, `bom-${sessionId}`);
    
    console.log("✅ BOM persisted successfully");
