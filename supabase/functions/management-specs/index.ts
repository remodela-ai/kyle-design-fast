...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Specs generated, persisting to storage...");
    
    // Persist the image to Supabase Storage
    const imageUrl = await persistImageToStorage(tempImageUrl, `specs-${sessionId}`);
    
    console.log("✅ Specs persisted successfully");
