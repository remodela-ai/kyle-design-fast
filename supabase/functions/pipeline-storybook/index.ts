...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Story Book generated, persisting to storage...");
     
    // Persist the image to Supabase Storage
    const imageUrl = await persistImageToStorage(tempImageUrl, `storybook-${sessionId}`);
    const description = "Story Book generated with Flux 2 Pro";
     
    console.log("✅ Story Book persisted successfully");
